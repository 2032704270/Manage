...
import chinaData from "./page.json";
export default {
  name: "BigDatas",
  mounted() {
    ...
    function countUp() {
      if (count === 0) {
        ...
        // @ts-ignore
        chart.data(handleData(Object.values(chinaData)[count]));
        ...
        chart.annotation().text({
          position: ["95%", "90%"],
          content: Object.keys(chinaData)[count],
          style: {
            fontSize: 40,
            fontWeight: "bold",
            fill: "#ddd",
        ...
        // @ts-ignore
        chart.changeData(handleData(Object.values(chinaData)[count]));
      }

      ++count;

      if (count === Object.keys(chinaData).length) {
